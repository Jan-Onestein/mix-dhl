...
class StockPicking(models.Model):
    ...
    save_request = fields.Boolean("Save Request")
    ...
    def button_print_dhl_label(self):
        ...
        try:
            ...
            auth_req = Request(
                ...
                data=request_xml.encode('ascii'),
                headers={'Content-Type': 'application/json'}
            )
            access_token = json.loads(urlopen(auth_req).read())['accessToken']
        except Exception as e:
            raise ValidationError(_("Error during authentication: %s" % e))
        ...
        for i in range (0,self.number_of_packages):
            ...
            nice_request_xml = json.dumps(json.loads(request_xml), indent=2, sort_keys=True)

            if self.save_request:
                self.env['ir.attachment'].create({
                    'name': self.name + '-' + str(i + 1) + '/' + str(self.number_of_packages) + '.xml',
                    'datas': base64.b64encode(nice_request_xml.encode('utf-8')),
                    'datas_fname': self.name + '-' + str(i + 1) + '/' + str(self.number_of_packages) + '.xml',
            ...
            try:
                ...
                raise ValidationError(_("Error during label creation: %s\nRequest is: %s" % (e, nice_request_xml)))
            ...
            try:
                ...
                raise ValidationError(_("Error during label retrieval: %s" % e))

            self.env['ir.attachment'].create({
                'name': self.name + '-' + str(i+1) + '/' + str(self.number_of_packages) + '.pdf',
                'datas': label_data,
                'store_fname': self.name + '-' + str(i+1) + '/' + str(self.number_of_packages) + '.pdf',
                'res_model': 'stock.picking',
                'res_id': self.id,
            })
